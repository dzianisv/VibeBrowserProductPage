...
const aggregateData = [
  { model: 'Grok 4', successRate: 98, avgTime: 2.1, interventions: 0.3, costPerRun: 0.012 },
  { model: 'GPT-5', successRate: 92, avgTime: 3.8, interventions: 1.1, costPerRun: 0.035 },
...
const taskData = [
  { task: 'Gmail Triage', 'Grok 4': 1.8, 'GPT-5': 3.2, 'Claude 4.6': 3.9, 'Ollama': 10.5 },
  { task: 'LinkedIn Outreach', 'Grok 4': 2.4, 'GPT-5': 4.1, 'Claude 4.6': 4.5, 'Ollama': 14.2 },
  { task: 'Price Scraping', 'Grok 4': 1.5, 'GPT-5': 2.9, 'Claude 4.6': 3.1, 'Ollama': 8.8 },
  { task: 'Form Filling', 'Grok 4': 2.8, 'GPT-5': 4.8, 'Claude 4.6': 5.2, 'Ollama': 15.3 },
  { task: 'Dashboard Nav', 'Grok 4': 2.0, 'GPT-5': 4.0, 'Claude 4.6': 4.3, 'Ollama': 11.2 },
]

const radarData = [
  { metric: 'Success Rate', 'Grok 4': 98, 'GPT-5': 92, 'Claude 4.6': 89, 'Ollama': 65 },
  { metric: 'Speed', 'Grok 4': 95, 'GPT-5': 72, 'Claude 4.6': 68, 'Ollama': 25 },
  { metric: 'Reliability', 'Grok 4': 97, 'GPT-5': 85, 'Claude 4.6': 82, 'Ollama': 45 },
  { metric: 'Cost Efficiency', 'Grok 4': 90, 'GPT-5': 60, 'Claude 4.6': 70, 'Ollama': 100 },
  { metric: 'Complex Tasks', 'Grok 4': 96, 'GPT-5': 88, 'Claude 4.6': 91, 'Ollama': 40 },
]
    ...
    <div className="my-8 h-[400px] w-full rounded-xl border border-[#3c4043] bg-[#2d2e30] p-4 sm:p-6">
      ...
      <ResponsiveContainer width="100%" height="85%">
        <LineChart data={taskData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
          ...
          <Line type="monotone" dataKey="Grok 4" stroke={colors.grok} strokeWidth={3} dot={{ r: 4 }} activeDot={{ r: 6 }} />
          <Line type="monotone" dataKey="GPT-5" stroke={colors.gpt} strokeWidth={3} dot={{ r: 4 }} />
    ...
    <div className="my-8 flex h-[450px] w-full flex-col items-center justify-center rounded-xl border border-[#3c4043] bg-[#2d2e30] p-4 sm:p-6">
      ...
      <ResponsiveContainer width="100%" height="90%">
        <RadarChart cx="50%" cy="50%" outerRadius="70%" data={radarData}>
          ...
          <Radar name="Grok 4" dataKey="Grok 4" stroke={colors.grok} fill={colors.grok} fillOpacity={0.3} strokeWidth={2} />
          <Radar name="GPT-5" dataKey="GPT-5" stroke={colors.gpt} fill={colors.gpt} fillOpacity={0.2} strokeWidth={2} />
      ...
      <table className="w-full border-collapse text-left text-sm text-[#e8eaed] sm:text-base">
        ...
        <tbody className="divide-y divide-[#3c4043]">
          <tr className="transition-colors hover:bg-[#2d2e30]/50">
            <td className="p-4 font-semibold text-[#8ab4f8]">Grok 4</td>
          ...
          <tr className="transition-colors hover:bg-[#2d2e30]/50">
            <td className="p-4 font-semibold text-[#81c995]">GPT-5</td>
