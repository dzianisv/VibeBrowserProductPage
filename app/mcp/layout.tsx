...
const faqJsonLd = {
  ...
  mainEntity: [
    ...
    },
    {
      ...
      acceptedAnswer: {
        '@type': 'Answer',
        text: 'Yes, both — via a connector rather than the header-based config. Claude on the web: Settings → Connectors → Add → Add custom connector, then paste the URL. ChatGPT on the web: turn on Developer mode under Settings → Security and login, then Plugins → Create app, which requires a paid plan. Neither requires domain verification, an allowlist, or a consent screen. The URL is the direct connector URL https://relay.api.vibebrowser.app/mcp/<your-routing-uuid>, copied from the Vibe extension; it is a bearer credential, so keep it private. That path is verified on Claude on the web, and on ChatGPT on a paid plan — on a free account Plugins → Create app silently no-ops, so the URL field never appears. Step-by-step guides are at vibebrowser.app/integrations/claude-connector and vibebrowser.app/integrations/chatgpt-connector.',
      },
    },
  ],
}
