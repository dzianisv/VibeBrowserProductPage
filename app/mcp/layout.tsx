...
const faqJsonLd = {
  ...
  mainEntity: [
    ...
    },
    {
      ...
      acceptedAnswer: {
        '@type': 'Answer',
        text: 'Yes, both — via a connector rather than the header-based config. Claude on the web: Settings → Connectors → Add → Add custom connector, then paste the URL. ChatGPT on the web: turn on Developer mode under Settings → Security and login, then Plugins → Create app, which requires a paid plan. Neither requires domain verification or an allowlist. The recommended URL is the canonical OAuth endpoint https://relay.api.vibebrowser.app/mcp — it contains no credential, the client registers itself via OAuth 2.1 Dynamic Client Registration, and you approve the browser:read and browser:control scopes once. That OAuth path is verified on Claude. On ChatGPT it is unverified, because Create app silently no-ops on a free account; the path verified on ChatGPT is the older per-user URL https://relay.api.vibebrowser.app/mcp/<your-routing-uuid>, which also suits headless clients that cannot show a consent screen. Step-by-step guides are at vibebrowser.app/integrations/claude-connector and vibebrowser.app/integrations/chatgpt-connector.',
      },
    },
  ],
}
