...
Both books are trading. The Claude book made its first committee decisions on day one — four buys (ETH, UNI, SOL, LINK) and a BTC hold under Extreme Fear conditions, each with a full thesis in the ledger. The DeepSeek book followed with its own first run: buys in BTC, ETH, SOL and AAVE, holds on the rest. The models already disagree — Claude bought UNI and LINK where DeepSeek held, DeepSeek bought BTC and AAVE where Claude passed. That disagreement, tracked over months on the same data, is the experiment. Both books, the decision log, and the live stats are at [quantarena.xyz/track-record](https://quantarena.xyz/track-record.html).

Getting DeepSeek's first run through was its own lesson in output contracts. On the first attempt it expressed "no action" on a token it didn't hold as `sell` with a zero allocation — our validator rejected the whole batch. Another attempt wrote `Aave` instead of the whitelisted `AAVE` — rejected again. No output was ever hand-fixed: we tightened the prompt's contract rules instead (no-action means `hold`, tickers must match exactly), and the next run passed clean. Strict validation plus prompt iteration beats loosening the validator every time.
